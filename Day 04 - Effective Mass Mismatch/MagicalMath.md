...
* $m^*$ is the **Effective Mass** ($0.067$ for $e$, $0.45$ for $h$).
* $L = 2\text{nm}$ (The specific Well Width used in `Simulation.py`).

#### **2. The Effective Mass Mismatch ($\Delta m$)**
The synchronization gap is mathematically represented by the ratio of their kinetic behaviors. Because the hole is significantly heavier, its energy levels are much lower, leading to a "mismatch" in state preparation:

$$\Delta m_{ratio} = \frac{m_e^*}{m_h^*} \approx 0.148$$

This ratio explains why the "lighter" electron responds more drastically to the confinement than the "heavy" hole.



#### **3. Recombination Probability ($P_{recomb}$)**
...
$$P_{recomb} \propto \left| \int_{0}^{L} \psi_e(z) \psi_h(z) \, dz \right|^2$$
...
In **Blue Team** operations for Quantum Networks, these equations aren't just theory—they are the parameters for our **Noise Models**. By calculating the exact energy mismatch, we can predict where the "Quantum Signal" is most vulnerable to environmental decoherence.
